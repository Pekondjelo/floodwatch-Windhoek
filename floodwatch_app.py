...
SHAPEFILE_NAME = "Windhoek_Urban.shp"  # Update if ZIP contains different .shp
...
try:
    ...
    if not SHAPEFILE_PATH.exists():
        ...
        try:
            ...
            st.write(f"Shapefile download status: {response.status_code}")  # Debug output
            if response.status_code != 200:
                raise Exception(f"Failed to download shapefile ZIP from Google Drive (status code: {response.status_code})")
            ...
            st.write(f"Files in data after extraction: {list(BASE_DIR.glob('*'))}")  # Debug output
            
            # Check for expected shapefile
            SHAPEFILE_PATH = BASE_DIR / SHAPEFILE_NAME
            if not SHAPEFILE_PATH.exists():
                # Look for any .shp file in data directory
                shp_files = list(BASE_DIR.glob("*.shp"))
                if shp_files:
                    SHAPEFILE_PATH = shp_files[0]
                    st.markdown(f'<p class="status-success">Found alternative shapefile: {SHAPEFILE_PATH.name}</p>', unsafe_allow_html=True)
                else:
                    raise FileNotFoundError(f"Shapefile {SHAPEFILE_NAME} not found, and no alternative .shp files found in {BASE_DIR}")
            ...
            response = requests.get(SHAPEFILE_ZIP_URL)
            if response.status_code != 200:
                raise Exception(f"Failed to download shapefile directly (status code: {response.status_code})")
            with open(SHAPEFILE_PATH, "wb") as f:
                f.write(response.content)
        
        if not SHAPEFILE_PATH.exists():
            raise FileNotFoundError(f"Shapefile {SHAPEFILE_NAME} not found after download attempts")
    ...
    if not DEM_PATH.exists():
        ...
        st.write(f"DEM download status: {response.status_code}")  # Debug output
        if response.status_code != 200:
            raise Exception(f"Failed to download DEM from Google Drive (status code: {response.status_code})")
        with open(DEM_PATH, "wb") as f:
            f.write(response.content)
    
    st.markdown('<p class="status-success">Data files ready (Shapefile: {SHAPEFILE_PATH.name})</p>', unsafe_allow_html=True)
...
try:
    ...
    rgba[flood_image == 1] = [255, 0, 0, 128]  # Red for flood-prone
    rgba[flood_image == 0] = [0, 0, 0, 0]      # Transparent for non-flood-prone
    ...
    # Add flood-prone areas as ImageOverlay
